# -*- coding: utf-8 -*-
from .models import Textos, Autores, Escalafh, Escalamu, Escalasp, Escalagu, Escalain, Cr, Fh, Gu, Mu, Sp, Ocasiones, \
    Ejes
...
class ChartData():
    ...
    def chart_data(indfil, autfil, ocfil, ejefil):
        ...
        if 'nombre' not in autfil.data:
            lista_autores = Autores.objects.all().values('id')
        else:
            lista_autores = autfil.data.getlist('nombre')
        autores = Autores.objects.filter(pk__in=lista_autores)
        if 'ocasion' not in ocfil.data:
            lista_ocasiones = Ocasiones.objects.all().values('pk')
        else:
            lista_ocasiones = ocfil.data.getlist('ocasion')
        if 'eje' not in ejefil.data:
            lista_ejes = Ejes.objects.all().values('pk')
        else:
            lista_ejes = ejefil.data.getlist('eje')
        ...
        for autor in data.get('autor'):
            ...
            if 'ocasion' not in ocfil.data and 'eje' not in ejefil.data:
                ...
                if 'ocasion' not in ocfil.data and 'eje' in ejefil.data:
                    lista_textos = Textos.objects.filter(idautor__nombre=autor, ideje__in=ejefil.data['eje']).order_by(
                        'fecha')
                else:
                    if 'ocasion' in ocfil.data and 'eje' not in ejefil.data:
                        lista_textos = Textos.objects.filter(idautor__nombre=autor,
                                                             idocasion__in=ocfil.data['ocasion']).order_by('fecha')
                    else:
                ...
                for texto in lista_textos:
                    if 'indice' not in indfil.data == {}:
                        ...
                        if '1' in indfil.data.getlist('indice'):  textodata = {'fecha': texto.fecha.__str__(),
                                                                               ...
                                                                               'name': texto.titulo}
                        if '2' in indfil.data.getlist('indice'):  textodata = {'fecha': texto.fecha.__str__(),
                                                                               'y': texto.gu.resultado,
                                                                               'name': texto.titulo}
                        if '3' in indfil.data.getlist('indice'):  textodata = {'fecha': texto.fecha.__str__(),
                                                                               'y': texto.fh.resultado,
                                                                               'name': texto.titulo}
                        if '4' in indfil.data.getlist('indice'):  textodata = {'fecha': texto.fecha.__str__(),
                                                                               'y': texto.mu.resultado,
                                                                               'name': texto.titulo}
                        if '5' in indfil.data.getlist('indice'):  textodata = {'fecha': texto.fecha.__str__(),
                                                                               'y': texto.sp.resultado,
                                                                               'name': texto.titulo}
